...
def count_floors_line(image_path, canny_low, canny_high, hough_threshold, tolerence, output_path, actual_floor_height = 10):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold = hough_threshold, minLineLength = 100, maxLineGap = 10)
    ...
    # Height calculation is simplified: (Number of Floors) * (Assumed Height per Floor)
    ...
    # # Annotate the Image: # Put the Answer on the image
    ...
    cv2.putText(img, f"Estimated Area : {area} Sq.Feets", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    # Return the result
    return floor_lines, num_floors, real_height, real_width

# The Inference Code
# canny low: 50, canny high: 150, hough_threshold = 80, tolerance = 130
if __name__ == "__main__":
    image_path = r"C:\Users\Webbies\Jupyter_Notebooks\Berger_Building_Height_Width\images\Image_3.jpg"
    output_path = r"C:\Users\Webbies\Jupyter_Notebooks\Berger_Building_Height_Width\images\Image_3_Result_Height_Width.jpg"
    
    # hough_threshold was increased to 100 in your latest script, keeping it that way
    floor_lines, floor_num, height, width, area = count_floors_line(image_path, canny_low = 50, canny_high = 150,  hough_threshold = 100, tolerence = 400 , output_path = output_path)
